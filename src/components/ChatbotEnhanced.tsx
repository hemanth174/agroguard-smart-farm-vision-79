...
const ChatbotEnhanced = () => {
  ...
  // Clear messages and reset state when chat is reopened
  useEffect(() => {
    if (isOpen) {
  ...
  // Enhanced AI response generator with session awareness
  const generateBotResponse = useCallback(async (userMessage: string, currentSessionId: string): Promise<string> => {
    console.log(`Generating response for session: ${currentSessionId}, message: ${userMessage}`);
    ...
    const responses = {
      en: {
        ...
        weather: `Current weather conditions are ideal for farming. Temperature: 28°C, Humidity: 65%, Wind: Light breeze. Today is perfect for field work, irrigation, and pest monitoring. Tomorrow's forecast shows similar conditions with a slight chance of evening showers. [Session: ${currentSessionId.slice(-6)}]`,
        ...
        market: `Current market rates: Rice ₹2,450/quintal (+₹50), Wheat ₹2,100/quintal (-₹25), Cotton ₹5,800/quintal (+₹100), Onion ₹3,200/quintal (+₹200), Tomato ₹4,500/quintal (-₹300). Prices updated at ${new Date().toLocaleTimeString()}. Best selling time: Early morning at mandis.`,
        ...
        default: `I can help you with: Crop cultivation, Weather guidance, Irrigation planning, Fertilizer recommendations, Pest & disease management, Market prices, Seed selection, Soil health, Equipment guidance, Government schemes, and much more. Please tell me what specific farming topic you'd like to know about? [Active session: ${currentSessionId.slice(-6)}]`
      },
      hi: {
        greeting: "नमस्कार! मैं आपकी सभी कृषि आवश्यकताओं में सहायता के लिए यहाँ हूँ। आप क्या जानना चाहते हैं?",
        weather: `वर्तमान मौसम स्थिति खेती के लिए आदर्श है। तापमान: 28°C, आर्द्रता: 65%, हवा: हल्की। आज खेत का काम, सिंचाई और कीट निगरानी के लिए बेहतरीन दिन है। कल का पूर्वानुमान भी ऐसा ही है, शाम को हल्की बारिश की संभावना। [Session: ${currentSessionId.slice(-6)}]`,
        ...
        market: `वर्तमान बाज़ार भाव: धान ₹2,450/क्विंटल (+₹50), गेहूं ₹2,100/क्विंटल (-₹25), कपास ₹5,800/क्विंटल (+₹100), प्याज ₹3,200/क्विंटल (+₹200), टमाटर ₹4,500/क्विंटल (-₹300)। भाव हर घंटे अपडेट। बेचने का सबसे अच्छा समय: सुबह मंडी में। [Session: ${currentSessionId.slice(-6)}]`,
        ...
        default: `मैं इनमें आपकी सहायता कर सकता हूँ: फसल उत्पादन, मौसम मार्गदर्शन, सिंचाई योजना, उर्वरक सुझाव, कीट व रोग प्रबंधन, बाज़ार भाव, बीज चुनाव, मिट्टी स्वास्थ्य, उपकरण मार्गदर्शन, सरकारी योजनाएं। कृषि के किस विषय पर जानकारी चाहिए? [Active session: ${currentSessionId.slice(-6)}]`
      },
      te: {
        greeting: "నమస్కారం! నేను మీ అన్ని వ్యవసాయ అవసరాలలో సహాయం చేయడానికి ఇక్కడ ఉన్నాను। మీరు ఏమి తెలుసుకోవాలనుకుంటున్నారు?",
        weather: `ప్రస్తుత వాతావరణ పరిస్థితులు వ్యవసాయానికి అనుకూలంగా ఉన్నాయి. ఉష్ణోగ్రత: 28°C, తేమ: 65%, గాలి: తేలికపాటి. ఈరోజు పొలం పని, నీటిపారుదల మరియు కీటక పర్యవేక్షణకు అద్భుతమైన రోజు. రేపటి అంచనా కూడా ఇలాగే ఉంది, సాయంత్రం చిన్న వర్షం అవకాశం. [Session: ${currentSessionId.slice(-6)}]`,
        ...
        market: `ప్రస్తుత మార్కెట్ ధరలు: వరి ₹2,450/క్వింటల్ (+₹50), గోధుమ ₹2,100/క్వింటల్ (-₹25), పత్తి ₹5,800/క్వింటల్ (+₹100), ఉల్లిపాయ ₹3,200/క్వింటల్ (+₹200), టమాటా ₹4,500/క్వింటల్ (-₹300)। ధరలు గంటకు ఒకసారి అప్‌డేట్. అమ్మకానికి మంచి సమయం: ఉదయం మార్కెట్‌లో. [Session: ${currentSessionId.slice(-6)}]`,
        ...
        default: `నేను వీటిలో మీకు సహాయం చేయగలను: పంట పండింపు, వాతావరణ మార్గదర్శకత్వం, నీటిపారుదల ప్రణాళిక, ఎరువుల సూచనలు, కీటక మరియు వ్యాధి నిర్వహణ, మార్కెట్ ధరలు, విత్తన ఎంపిక, మట్టి ఆరోగ్యం, పరికరాల మార్గదర్శకత్వం, ప్రభుత్వ పథకాలు. వ్యవసాయానికి సంబంధించిన ఏ అంశంపై సమాచారం కావాలి? [Active session: ${currentSessionId.slice(-6)}]`
      }
    };

    const currentResponses = responses[language] || responses.en;

    // Enhanced keyword matching with session-aware responses
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    console.log(`Sending message: "${currentInput}" in session: ${currentSession}`);
    ...
    setTimeout(async () => {
      try {
        const response = await generateBotResponse(currentInput, currentSession);
  ...
  }, [inputMessage, isTyping, sessionId, addUserMessage, addBotMessage, generateBotResponse]);

  const startVoiceRecognition = useCallback(() => {
    if (recognitionRef.current && isOnline && !isListening) {
      setIsListening(true);
      try {
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3 flex-shrink-0 bg-green-600 text-white rounded-t-lg">
          <div>
            ...
            <Badge className="bg-green-100 text-green-800 hover:bg-green-100 text-xs">
              {isOnline ? 'Online' : 'Offline'} • {sessionId.slice(-6)}
            </Badge>
          </div>
          <Button
            variant="ghost"
          ...
          <div className="p-4 border-t bg-gray-50 flex-shrink-0">
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={!isOnline || !recognitionRef.current || isListening || isTyping}
